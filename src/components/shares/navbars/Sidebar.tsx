...
function Sidebar({
  ...
  return (
    <div className="flex flex-col px-2 py-4 bg-clr-background-base-two rounded-lg gap-2 col-span-1 row-span-6 font-medium">
      {Object.entries(navList).map(([url, [displayName, iconUrl]]) => (
        <Link
          to={url}
          className={`flex justify-start gap-6 items-center opacity-70 cursor-pointer py-2 px-2 font-bold ${currentUrl === url ? 'opacity-100' : ''}`}
